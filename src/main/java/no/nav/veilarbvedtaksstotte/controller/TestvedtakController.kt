...
class TestvedtakController(
    ...
    fun fattTestVedtak(@RequestBody opprettTestvedtakRequest: OpprettTestvedtakRequest, @RequestHeader("Nav-Callid") navCallId: String) {
        /*if (!authService.harSystemTilSystemTilgangMedEkstraRolle("fatt-14a-vedtak") || navCallId.isBlank()) {
        ...
        if (EnvironmentUtils.isDevelopment().orElse(false)) {
            ...
            testvedtakService.lagreTestvedtak(opprettTestvedtakRequest.toVedtak(aktorId), opprettTestvedtakRequest.fnr, navCallId)
            return
        }
        throw ResponseStatusException(HttpStatus.FORBIDDEN, "Fatt vedtak er ikke støttet i produksjon. Denne funksjonaliteten er kun tilgjengelig i preprod-miljøer for testing av ny vedtaksløsning." +
    ...
    fun hentTestVedtak(@RequestBody testvedtakRequest: TestvedtakRequest): Vedtak? {
       /* if (!authService.harSystemTilSystemTilgangMedEkstraRolle("fatt-14a-vedtak")) {
        ...
        throw ResponseStatusException(HttpStatus.FORBIDDEN,"Henting av vedtak er ikke støttet i produksjon. Denne funksjonaliteten er kun tilgjengelig i preprod-miljøer for testing av ny vedtaksløsning.")
    }
    ...
    fun slettTestVedtak(@RequestBody testvedtakRequest: TestvedtakRequest) {
       /* if (!authService.harSystemTilSystemTilgangMedEkstraRolle("fatt-14a-vedtak")) {
        ...
        throw ResponseStatusException(HttpStatus.FORBIDDEN,"Sletting av vedtak er ikke støttet i produksjon. Denne funksjonaliteten er kun tilgjengelig i preprod-miljøer for testing av ny vedtaksløsning.")
    }
}